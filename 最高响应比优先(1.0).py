...
def hrrn(processes_):
        ...
        running_time = 0
        flag = 0
        pre_processes.append(sum_processes[0])
        while(flag!=len(sum_processes)):
            # 计算响应比
            ...
            for i in range(pre_processes[max_rr].serve_time):
                running_time += 1
                for k in range(len(sum_processes)):  # 就绪队列入队
                    if (sum_processes[k].arrive_time == running_time):
                        pre_processes.append(sum_processes[k])
            
            # 计算属性
            pre_processes[max_rr].cycling_time = running_time - pre_processes[max_rr].arrive_time
            pre_processes[max_rr].w_cycling_time = pre_processes[max_rr].cycling_time / pre_processes[max_rr].serve_time
            sum_cycling_time += pre_processes[max_rr].cycling_time
            sum_w_cycling_time += pre_processes[max_rr].w_cycling_time
...
class ui(QWidget):
    ...
    def g_info(self):
        ...
        infor_list = hrrn(_sorted_processes)
