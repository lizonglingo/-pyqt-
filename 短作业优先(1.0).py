...
def sjf(processes_):
        ...
        while(flag!=5):
            if current_process != -1:  # 此时有进程在执行
                ...
                if processes[current_process].used_time == processes[current_process].serve_time:
                    print('进程' + processes[current_process].name + '处理完毕！')

                    # 计算相关的参数
                    processes[current_process].cycling_time = time + processes[current_process].arrive_time
                    processes[current_process].w_cycling_time = processes[current_process].cycling_time / processes[current_process].serve_time
                    sum_cycling_time += processes[current_process].cycling_time
                    sum_w_cycling_time += processes[current_process].w_cycling_time
                    
                    flag += 1
                    over_list.append(processes[current_process])
            ...
            for i in range(len(processes)):
                if processes[i].ready == True and processes[i].over == False:
                    if current_process == -1:
                        min_handle = processes[0].serve_time
                        current_process = 0
                    else:
                        if processes[current_process].over == True:
                            for i in range(len(processes)):
                                if processes[i].ready == True and processes[i].over == False:
                                    if  processes[i].serve_time <= min_handle:
                                        current_process = i
                                        min_handle = processes[i].serve_time
            time += 1
...
class ui(QWidget):
    ...
    def g_info(self):
        ...
        infor_list = sjf(_sorted_processes)
